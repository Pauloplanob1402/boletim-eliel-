...
/**
 * Cria uma assinatura (preapproval) SEM vínculo a um plano do Mercado Pago.
 *
 * Importante: assinaturas vinculadas a um preapproval_plan_id exigem
 * card_token_id (cartão tokenizado via Checkout Bricks no seu site). Como
 * este fluxo redireciona o assinante para escolher a forma de pagamento no
 * próprio Mercado Pago (sem coletar cartão aqui), os dados de recorrência
 * (reason/auto_recurring) precisam ir direto no corpo da requisição — é o
 * único jeito de receber um init_point de redirecionamento sem card_token_id.
 *
 ...
 */
export async function createSubscription({ reason, frequency, frequencyType, amount, payerEmail, externalReference }) {
